...
$('.start_practice').click(function(){
    ...
    //start it again 
    $("#re_load").html('<input type ="submit"  class="re_start" id="practice_reload" value="Reload to Start a New Test"><p>Pick up the correct answer choice listed below the word. Click the "Next Word" button to answer another question. Before clicking the start button to go to a new question, always click "check answer for this word" button to check whether your answer is correct, unless you want skip  the  question.</p>');
...
/*----------------------------------------------------------------------------------------
    Produce a word for testing and the four explanation choices and print it out
----------------------------------------------------------------------------------------*/
    ...
    if(practiceVocabulary.length>=50){
    		random_number0 = start_number + Math.floor(Math.random()*words_count)
    		}
    		else{
    		random_number0 = start_number + Math.floor(Math.random()*practiceVocabulary.length)
    		};
